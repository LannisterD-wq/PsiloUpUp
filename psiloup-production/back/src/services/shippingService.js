...
const { calculateCorreiosShipping } = require('./correiosService');
...
async function quote({ destinationCep, items: rawItems }) {
  ...
  // 1. Tenta Correios primeiro (API pública, não precisa de token)
  try {
    const correiosQuote = await calculateCorreiosShipping({ destinationCep, items });
    if (correiosQuote && correiosQuote.services && correiosQuote.services.length > 0) {
      return correiosQuote;
    }
  } catch (error) {
    console.warn('Erro ao calcular frete dos Correios, tentando Melhor Envio:', error.message);
  }
  
  // 2. Fallback para Melhor Envio (se tiver token)
  const melhorEnvio = await getMelhorEnvioQuote({ destinationCep, items });
  if (melhorEnvio) {
    return melhorEnvio;
  }
  
  // 3. Fallback final (cálculo simples)
